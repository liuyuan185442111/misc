...
public:
	...
	public:
		...
		static ThreadContext *GetInstance()
		{
			...
			if(!spec) pthread_setspecific(key, spec = new ThreadContext());
			return spec;
		}
	};
	...
	public:
		...
		void checkcommit(bool cancel)
		{
			...
			{
				fprintf(stderr, "storagewdb::checkcommit DISK ERROR! PROGRAM ABORT!\n");
				abort();
			}
		}
		...
		void insert( const GNET::Octets &key, const GNET::Octets &val, Transaction &txn, int flags = 0 )
		{
			...
			{
				fprintf(stderr, "storagewdb::insert DISK ERROR! PROGRAM ABORT!\n");
				abort();
			}
		}
		...
		GNET::Octets find( const GNET::Octets &key, Transaction &txn )
		{
			...
			{
				fprintf(stderr, "storagewdb:find DISK ERROR! PROGRAM ABORT!\n");
				abort();
			}
			return GNET::Octets();
		}
		...
		void del( const GNET::Octets &key, Transaction& txn, int flags = 0 )
		{
			...
			{
				fprintf(stderr, "storagewdb:del DISK ERROR! PROGRAM ABORT!\n");
				abort();
			}
		}
		...
		class Cursor
		{
			...
			public:
				...
				bool update( const void *key, size_t key_len, const void *val, size_t val_len )
				{
					...
					catch ( GNET::OctetsStream::Exception e ) { }
					catch ( ... ) { }
...
public:
	...
	static bool Open()
	{
		...
		SplitString(tables, Conf::find("storagewdb", "tables").c_str(), ",");
		...
		if ( env->init() )
		{
			for( size_t k=0; k<tables.size(); k++ )
			{
				Storage *storage = new Storage((datadir + "/" + tables[k]).c_str(), env->db(tables[k].c_str()),new NullCoder(),new NullCoder());
	...
	static bool checkpoint()
	{
		...
		{
			GNET::Thread::Mutex::Scoped l(checkpoint_locker);
			//if(!checkpoint_locker.TryLock()) return true;
	...
	static void removeoldlogs( )
	{
		if( env )
		{
			std::vector<GNET::__db_helper::time32_t> r = env->remove_logs();
			char *name = (char *)malloc( logdir.length() + 32 );
			std::vector<GNET::__db_helper::time32_t>::iterator it, ite;
			for( it = r.begin(), ite = r.end(); it != ite; ++it )
			{
				sprintf(name, "%s/log.%08x", logdir.c_str(), *it);
				unlink( name );
			}
			free(name);
		}
	...
	static void * BackupThread( void * pParam )
	{
		...
		while( true )
		{
			try
			{
				...
				{
					fprintf(stderr, "storagewdb:BackupThread DISK ERROR! PROGRAM ABORT!\n");
					//abort();
				}
				puts( "checkpoint end." );
...
{
	if(!locked) return;
	std::for_each(storage_vec.begin(), storage_vec.end(), std::bind2nd(std::mem_fun(&Storage::checkcommit),cancel));
	//storage_vec.clear();
	locked = false;
}
