...
inline time32_t time32_now() { return (time32_t)time(NULL); }
...
public:
	...
	/*
	@return   int &pos    result
	NULL       N/A        NOT FOUND
	NOT NULL   0          FOUND:   equals(key)
	NOT NULL   >0         GREATER: the greater after key
	NOT NULL   <0         GREATER: the greater after key, key is less than all key in table
	*/
	...
	index_hdr* great_equal_key( const void *key, size_t key_len )
	{
		int pos;
		index_hdr* hdr=find_key( key, key_len, pos ); 
		return ( hdr!=NULL && hdr==Layout2Page(hdr)->index_tail() ) ? NULL : hdr;
	...
	index_hdr* greater_key( const void *key, size_t key_len )
	{
		...
		if (index_hdr *hdr = find_key( key, key_len, pos ))
		{
			if (pos == 0) // equals found.
				return next(hdr); // return NULL if the key is last. see next.
			// pos < 0 see next line.
			if (hdr != Layout2Page(hdr)->index_tail()) // greater found. maybe tail. see great_equal_key.
				return hdr;
		}
		// eof
		return NULL;
...
public:
	...
	// safe next_key
	void* greater_key( const void *key, size_t &key_len )
	{
		if (index_hdr *hdr = root_index_page()->greater_key( key, key_len ))
			return extract_key(hdr->Rindex_hdr2data_hdr(), key_len);
		return NULL;
	}
...
class GlobalLogger : public Logger
{
	...
	bool read_log_file( time32_t new_time_stamp )
	{
...
public:
	...
	std::vector<PageLayout *> read_magic( time32_t timestamp, time32_t start, const char *dbfile )
	{
	...
	time32_t get_time_stamp() const { return time_stamp; }
...
public:
	...
	const Performance *performance_from_begin( time32_t *t_org, time32_t *t_new ) const { *t_org = time_org; *t_new = time_new; return &perf_org; }
	const Performance *performance_from_checkpoint( time32_t *t_cur, time32_t *t_new ) const { *t_cur = time_cur; *t_new = time_new; return &perf_cur; }
};
...
public:
	...
	LoggerMonitor( PerformanceMonitor *monitor, const char *dbfile, const char *logdir, time32_t t_start = 0 ) : performance_monitor(monitor)
	{
		std::vector<time32_t> r;
		if ( DIR *dir = opendir( logdir ) )
		{
			while ( dirent *d = readdir( dir ) )
				...
				{
					time32_t ts;
					(void)sscanf( d->d_name, "log.%x", &ts );
					r.push_back( ts );
				}
			closedir( dir );
		}
		std::sort( r.begin(), r.end() );
		std::vector<time32_t>::iterator it = std::lower_bound( r.begin(), r.end(), t_start );
		if ( it != r.end() )
		{
			if ( it != r.begin() )
				--it;
...
public:
	...
	bool init()
	{
		...
		if ( state != PageLogger::CLEAN )
		{
			if ( state == PageLogger::CORRUPT )
				...
				state = logger->redo( __db_helper::time32_now() );
			delete page_cache;
			if ( state == PageLogger::CLEAN )
				page_cache = new PageCache(page_file, cache_high, cache_low);
		}
	...
	bool checkpoint_commit()  { return logger->commit(__db_helper::time32_now()) == Logger::CLEAN; }
	bool checkpoint()
	{
		snapshot_create();
...
public:
	...
	std::vector<__db_helper::time32_t> version () { return logger->check_version(); }
	std::vector<__db_helper::time32_t> restore (__db_helper::time32_t timestamp) { return logger->restore( timestamp ); }
	std::vector<__db_helper::time32_t> remove_logs()
	{
		std::vector<__db_helper::time32_t> r;
		__db_helper::time32_t time_stamp = logger->get_logger_id();
		if ( DIR *dir = opendir( logdir ) )
		{
			while ( dirent *d = readdir( dir ) )
				if ( d->d_name[0] != '.' )
				{
					__db_helper::time32_t ts;
