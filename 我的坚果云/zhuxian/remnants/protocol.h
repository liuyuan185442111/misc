...
public:
	...
	public:
		...
		protected:
			...
			Session(Manager *m) : sid(nextsid()), is(this), os_bytes(0), manager(m), state(m->GetInitState()), verbose(LOG_ERR){ }
			Session(const Session &rhs) : NetSession(rhs), sid(nextsid()), is(this), os_bytes(rhs.os_bytes), manager(rhs.manager),
						      state(rhs.state), verbose(rhs.verbose){ }
		private:
	...
	public:
		...
		virtual bool CheckIgnore(Protocol::Type) { return false; }
	};
private:
...
private:
	static void CheckPolicy(Type type, unsigned int size, const Manager::Session::Stream &is)
	{
		...
		{
			// origin check
			if ((!is.session->StatePolicy(type) || !is.session->manager->InputPolicy(type, size)) && !is.session->manager->CheckIgnore(type))
			{
				Log::log(LOG_ERR, "Protocol state or size policy error. sid=%d,type=%d,size=%d,acceptsize=%d",
		...
		{
			// unkown check
			if (!is.session->manager->CheckUnknownProtocol(is.session->GetSid(), type, size) && !is.session->manager->CheckIgnore(type))
			{
				Log::log(LOG_ERR, "Protocol Unknown . sid=%d,type=%d,size=%d,acceptsize=%d",
						is.session->GetSid(),type,size,is.session->AcceptSize());
				throw Protocol::Exception();
			}
		}
	...
	static Protocol *Decode(const Manager::Session::Stream &is)
	{
		...
		try
		{
			...
			if (is.session->manager->CheckIgnore(type))
			{
