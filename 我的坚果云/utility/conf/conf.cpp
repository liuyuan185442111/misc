#include "conf.h"
#include <sys/stat.h>
#include <unistd.h>
#include <map>
#include <vector>
#include <cstring>
#include <fstream>

namespace lcore {
namespace {
using std::string;
typedef string section_type;
typedef string key_type;
typedef string value_type;

struct stringcasecmp
{
	bool operator() (const string &x, const string &y) const { return strcasecmp(x.c_str(), y.c_str()) < 0; }
};
typedef std::map<key_type, value_type, stringcasecmp> section_map;
typedef std::map<section_type, section_map, stringcasecmp> conf_map;

class _Conf
{
	conf_map confmap;
	time_t mtime;
	string filename;
	string lastfile;
	string null;
	_Conf() : mtime(0) { }
	bool reload()
	{
		struct stat st;
		if(stat(filename.c_str(), &st) != 0) return false;
		for(; lastfile != filename || mtime != st.st_mtime; stat(filename.c_str(), &st))
		{
			lastfile = filename;
			mtime = st.st_mtime;
			std::ifstream ifs(filename.c_str());
			string line;
			section_type section;
			section_map secmap;
			if(!confmap.empty()) confmap.clear();
			while(std::getline(ifs, line))
			{
				if(line[0] == '#' || line[0] == ';') continue;
				if(line[0] == '[')
				{
					string::size_type start = line.find_first_not_of(" \t", 1);
					if(start == string::npos) continue;
					string::size_type end   = line.find_first_of(" \t]", start);
					if(end   == string::npos) continue;
					if(!section.empty()) confmap[section] = secmap;
					secmap.clear();
					section = section_type(line, start, end - start);
				}
				else
				{
					string::size_type key_start = line.find_first_not_of(" \t");
					if(key_start == string::npos) continue;
					string::size_type key_end   = line.find_first_of(" \t=", key_start);
					if(key_end   == string::npos) continue;
					string::size_type val_start = line.find_first_of("=", key_end);
					if(val_start == string::npos) continue;
					val_start = line.find_first_not_of(" \t", val_start + 1);
					if(val_start == string::npos) continue;
					string::size_type val_end = line.find_last_not_of(" \t\r\n");
					if(val_end   == string::npos) continue;
					if(val_end < val_start) continue;
					secmap[key_type(line, key_start, key_end-key_start)] = value_type(line, val_start, val_end-val_start+1);
				}
			}
			if(!section.empty()) confmap[section] = secmap;
		}
	}
public:
	static _Conf instance;
	bool load(const char *file)
	{
		if(file)
		{
			if(access(file, R_OK) == 0)
			{
				instance.filename = file;
				return instance.reload();
			}
			return false;
		}
		if(instance.lastfile.empty()) return false;
		if(access(instance.lastfile.c_str(), R_OK) == 0)
		{
			return instance.reload();
		}
		return false;
	}
	value_type &find(const section_type &section, const key_type &key)
	{
		if(instance.lastfile.empty()) return null;
		conf_map::iterator it = confmap.find(section);
		if(it != confmap.end())
		{
			section_map::iterator it2 = it->second.find(key);
			if(it2 != it->second.end()) return it2->second;
		}
		return null;
	}
	section_map &operator[](const section_type &section) { return confmap[section]; }
};
_Conf _Conf::instance;
}

bool Conf::load(const char *file)
{
	return _Conf::instance.load(file);
}
value_type Conf::find(const section_type &section, const key_type &key)
{
	return _Conf::instance.find(section, key);
}

Conf::helper Conf::operator[](const string &section)
{
	return helper((void*)(&_Conf::instance[section]));
}
string &Conf::helper::operator[](const string &key)
{
	return (*(section_map*)pdata)[key];
}
}
