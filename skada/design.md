...
## bug
少数情况下目标的名字可能会显示为空
四级界面中显示的是处于固定次序的条目的相关信息，也就是说如果三级界面中条目顺序发生变化，四级界面（当前界面）中显示的内容就会跟着改变
...
如果名字当中存在特殊字符比如单双引号和转义字符会不会有问题
做一个全局的名字和职业索引, lua获取名字和职业由这个换存取
确认一下技能或buff的id为0的情况是个什么情况
cpp里battle的有效性由lua来决定，战斗结束，lua在进行存盘的同时，告知cpp中有效的战斗，删掉多余的战斗
存盘时 筛选出buff统计里用的玩家的名字和职业 然后也存盘
cpp里的0号战斗存不存呢？不存了，因为要经常删也不一定用得着
分两个版本号 可以处理lua数据结构未改变 但buff结构改变的情况
