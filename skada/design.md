...
再次出现0号技能, 如果已经打开窗口了，出现0号技能弹窗提醒
获取某人信息获取不到，查看一下c++代码，是否和无敌有关，同样如果已经打开窗口了，出现无法获取的情况，及时弹窗提醒
能不能将tip界面和下一级界面的逻辑整合起来 他们的不同有：展示数量不同，设置目标不同，标题设置目标不同, 标题内容不同
将刷条目的逻辑封装起来 接受参数有 最大展示数量(用迭代器循环，达到数量上限退出循环)，目标list
全局的右键返回
