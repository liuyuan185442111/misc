//逻辑运算, 支持小括号, 1为true, 0为false, +为与, *为或, !为非
//表达式合法方可正常计算, 没有处理碰到')'异常的情况(此时程序可能崩溃)
#include <iostream>
#include <string>
#include <stack>
...
{
	if(oper_stk.empty()) return false;
	char oper = oper_stk.top();
	oper_stk.pop();
	if(oper == '!')
	{
		if(num_stk.empty()) return false;
		int num = num_stk.top();
		num_stk.pop();
		num_stk.push(num == 0 ? 1 : 0);
		std::cout << "caculate: " << oper << num << "=" << (num == 0 ? '1' : '0') << std::endl;
	}
	else
	{
		if(num_stk.size() < 2) return false;
